...
import requests
import json
from info import DATABASE_URI, DATABASE_NAME, COLLECTION_NAME, USE_CAPTION_FILTER, AUTH_CHANNEL, API_KEY,ADMINS
...
async def get_req(user_id):
        return await db.req.find_one({"user_id": int(user_id)})

async def is_subscribed(bot, message):
    user = await get_req(message.from_user.id)
    if user:
        return True
